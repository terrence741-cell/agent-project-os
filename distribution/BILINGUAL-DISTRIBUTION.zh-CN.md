...
- 6 个 Skill 名称及其路由关系；
- 按需触发边界，以及不介入普通项目工作的说明；
- 首次使用示例，包括用户不必先学习 Skill 名的规则；
- 读/写边界和确认门；
- 必须输出的结构与安全护栏；
...
5. 核验两个仓库 Marketplace 清单，以及 Codex 与 Claude Code 的全新安装说明。
6. 针对两种语言呈现分别运行阻断发布的行为用例。
7. 从同一个 Git tag 发布两个包；否则两个包均不发布。
