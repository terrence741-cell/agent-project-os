...
对全新用户，默认提议一个工具中立的唯一物理库：`~/.agent-project-os/shared-memory/`。`MEMORY.md` 只做薄总路由，用户画像、团队、项目、反馈和参考说明按已确认内容分文件延伸。Codex 与 Claude Code 的薄入口指向同一物理库，不用复制或双写同步。路径、文件、宿主规则、软链接、已实证需要的宿主读写权限与本地 Git 恢复都会先分开列明，再由你确认。
...
当前分支是 V1.0.2 发布候选。仓库已同时提供两种宿主的 Marketplace 清单，但最终公开 GitHub 安装命令要等第 4 阶段用真实仓库 URL 在全新 Codex 和 Claude Code 环境中验证后才发布。在此之前，请通过宿主的本地 Marketplace／插件源流程添加本仓库，并且只选择一个语言包。
